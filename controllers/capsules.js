...
const getCapsules = async (req, res) => {
  try {
    const capsule = await db.query(
      "SELECT c.id, poi.id, capsule_name, description, duration_video, link, url_video, poi_name, address, latitude, longitude FROM capsule c INNER JOIN poi ON c.id_poi=poi.id"
    );
    return res.status(200).json(capsule[0]);
  } catch (e) {
    console.log(e);
